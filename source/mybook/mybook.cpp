...
namespace MyBook
{
	...
	//   book_body[sfen] = ptr;
	// と等価。
	void MemoryBook::append(const std::string& sfen, const MyBook::BookMovesPtr& ptr)
	{
		std::lock_guard<std::recursive_mutex> lock(mutex_);
		book_body[sfen] = ptr;
	}
	...
	bool BookMoveSelector::probe_impl(Position& rootPos, bool silent, Move16& bestMove, Move16& ponderMove, bool forceHit)
	{
		...
		if (forceHit)
		{
			...
			auto it_end = std::remove_if(move_list.begin(), move_list.end(), [&](MyBook::BookMove& m) { return m.value < value_limit; });
			...
			if (narrowBook)
			{
				...
				// 出現確率10%未満のものを取り除く。
				auto it_end = std::remove_if(move_list.begin(), move_list.end(), [&](MyBook::BookMove& m)
