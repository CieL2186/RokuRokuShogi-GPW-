...
#if defined(USE_MATE_1PLY) && defined(MINISHOGI) && !defined(LONG_EFFECT_LIBRARY)
...
namespace {
	...
	void init_check_bb()
	{
		...
		for (PieceType p : { PAWN, SILVER, BISHOP, ROOK, GOLD, KING })
			for (auto sq : SQ)
				for (auto c : COLOR)
				{
					Bitboard bb = ZERO_BB, tmp = ZERO_BB;
			...
			if (!(dr & DIRECTIONS_DIAG)) // (pt == ROOK || pt == LANCE)
			{
				...
				if (is_rook || is_dragon)
				{
					...
					while (bb)
					{
						...
						if (canPromote(Us, from, to) || type_of(pos.piece_on(from)) == DRAGON)
...
NEXT1:;
	...
	bb = check_cand_bb(Us, PIECE_TYPE_CHECK_NON_SLIDER, sq_king)
		& (pos.pieces(Us, GOLDS, SILVER, PAWN));
	if (!bb)
		goto DC_CHECK;
	...
	while (bb)
	{
		...
		while (bb_check)
		{
			...
			return make_move(from, to, pos.piece_on(from));
		}
	}
