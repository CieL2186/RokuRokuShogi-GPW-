...
//			MoveList<QUIET_CHECKS_ALL> mqc(pos);
...
//						m = pos.move16_to_move(m);
						...
						{
							cout << endl << pos.pseudo_legal(m) << ' ' << pos.legal(m);
							cout << endl << pos.moved_piece_after(m) << endl;
...
#if defined (PACKED_SFEN_CHECK)
			{
				PackedSfen sfen;
				pos.sfen_pack(sfen);
				ASSERT_LV3(pos.sfen_unpack(sfen) == pos.sfen(0));
			}
#endif
